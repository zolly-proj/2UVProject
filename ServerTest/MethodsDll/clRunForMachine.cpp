...
#include "clRunForMachine.h"
clRunForMachine::clRunForMachine()
{
    try
    {		
		meSocket = NULL;
...
int clRunForMachine::GetReturnParameters()
{
    try
    {
		return 0;
...
bool clRunForMachine::doMethod(const vector <QString> &paParametersType, const vector <QString> &paParameters)
{
	try
	{
		if (meSocket == NULL)
		{
			...
			if(!loGetById)
			{
				meIceClientLogging->insertItem("10",QString(QHostInfo::localHostName()),"2UVServerTest.exe","clRunForMachine::doMethod -> " + loReturnMessageGetById);
				return false;
			}
			else
				meIceClientLogging->insertItem("10",QString(QHostInfo::localHostName()),"2UVServerTest.exe","clRunForMachine::doMethod -> " + loReturnMessageGetById);
			...
			{
				meIceClientLogging->insertItem("10",QString(QHostInfo::localHostName()),"2UVServerTest.exe","clRunForMachine::doMethod() -> cannot connect to host");			
				delete meSocket;
				meSocket = NULL;
				return false;
			}
		...
		meIceClientLogging->insertItem("10",QString(QHostInfo::localHostName()),"2UVServerTest.exe",QString("clRunForMachine::doMethod() command send to machine -> " + loCommandSend));
		meSocket->waitForBytesWritten(50);
		meSocket->waitForReadyRead(300);
		
		meData = meSocket->readAll();
		meIceClientLogging->insertItem("10",QString(QHostInfo::localHostName()),"2UVServerTest.exe",QString("clRunForMachine::readTcpData() command recieved from machine -> " + QString::fromLocal8Bit(meData.constData())));
		return true;
	}
	catch(...)
	{
...
bool clRunForMachine::createPluginClass( clIceClientServer * paIceClientServer, clIceClientLogging * paIceClientLogging)	
{
    try
    {
