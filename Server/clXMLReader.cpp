				...
				if (!loTablesElem.isNull())
				{
					...
					while(!loTableNode.isNull())
					{
						...
						if(!loRoutineElem.isNull())
						{
							...
							{
								QString loTemp("clXMLReader::readDatabaseConfiguration(QString paFileName, vector<clDatabaseTable> & paDatabaseTable, QString &paReturnMessage) -> routine tag [" +  QString("%1").arg(j) + "]  no returtype for node  ...");
								paReturnMessage = loTemp;
								return false;
							}
