...
bool clListWidgetExtendForObject::getCurrentValue(QString paUUID)
{
	try
    {
		if (paUUID.compare(QString("")) == 0)
		{
			meListWidget->addItem(QString(""));
		}
		else
		{
			...
			if (loClasses.size() > 1)
			{
				...
				for (int j = 0; j < loClasses.size(); j++)
				{
					...
					QString loClassName = QString(loClasses.at(j));
					...
					{
						meIceClientLogging->insertItem("10",QString(QHostInfo::localHostName()),"2UVServerTest.exe","clListWidgetExtendForObject::getCurrentValue() -> " + loReturnMessageGetById);
					}
					else
					{
						meIceClientLogging->insertItem("10",QString(QHostInfo::localHostName()),"2UVServerTest.exe","clListWidgetExtendForObject::getCurrentValue() -> " + loReturnMessageGetById);
						...
						{
							meListWidget->addItem(loReturnValues[i].c_str());
							loFound = true;
						}									
					}
			...
			else
			{
				...
				else
				{
					...
					for (int i = 0; i < loReturnValues.size(); i++)
					{
						meListWidget->addItem(loReturnValues.at(i).c_str());
						break;
					}
				}
			}
    ...
    {		
        meIceClientLogging->insertItem("10",QString(QHostInfo::localHostName()),"2UVServerTest.exe","clListWidgetExtendForObject::getCurrentValue() -> " + QString(e.what()));
		return false;
    }	
}
 ...
 void clListWidgetExtendForObject::createGridGroupBox()
 {
    ...
    meHBoxLayout[1]->addWidget(meButtons[3]);
	 ...
	 meListWidgetId = new QListWidget;
	 meListWidgetId->setSelectionMode(QAbstractItemView::SingleSelection);
...
void clListWidgetExtendForObject::slotButtonDeletePressed()
{
    try
    {
		...
		int row = meListWidget->currentRow();
		meListWidgetId->takeItem(row);
    }
    catch(exception &e)
    {

        meIceClientLogging->insertItem("10",QString(QHostInfo::localHostName()),"2UVServerTest.exe","clListWidgetExtendForObject::slotButtonDeletePressed() -> " + QString(e.what()));
    }
}
...
void clListWidgetExtendForObject::slotButtonAddPressed()
{
    try
    {
		...
		if (loListWidgetSelectObject->exec() == 0)
        {
			...
			meListWidget->addItem(QString(loListWidgetSelectObject->meNames[0]->toStdString().c_str()));
			meListWidgetId->addItem(QString(loListWidgetSelectObject->meValues[0]->toStdString().c_str()));
														
            meIceClientLogging->insertItem("10",QString(QHostInfo::localHostName()),"2UVServerTest.exe",QString("clListWidgetExtendForObject::slotButtonAddPressed() -> " + loReturnMessage));
        }
        else
        {
            meIceClientLogging->insertItem("10",QString(QHostInfo::localHostName()),"2UVServerTest.exe","clListWidgetExtendForObject::slotButtonAddPressed() -> insert canceled");
        }
    }
    catch(exception &e)
    {

        meIceClientLogging->insertItem("10",QString(QHostInfo::localHostName()),"2UVServerTest.exe","clListWidgetExtendForObject::slotButtonCancelPressed() -> " + QString(e.what()));
    }
}
void clListWidgetExtendForObject::slotButtonMoveUp()
{
    try
    {
		...
		if (loRow != 0)
		{
			...
			QListWidgetItem *loListWidgetItemTempId = meListWidgetId->takeItem(loRow);
			meListWidgetId->insertItem(loRow - 1, loListWidgetItemTempId);			
		}		
    }
    catch(exception &e)
    {
