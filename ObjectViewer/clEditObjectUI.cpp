...
void clEditObjectUI::initializeElements()
{
 ...
 void clEditObjectUI::createGridGroupBox(vector <std::string> paValues)
 {
			...
			*/
			 ...
			 for (int i = 0; i < meDatabaseColumns.size(); ++i) {
				 ...
				 {	
					 ...
					 {
                         meIceClientLogging->insertItem("10",QString(QHostInfo::localHostName()),"2UVServerTest.exe","clEditObjectUI::createGridGroupBox() -> meLineEdit");
						 ...
						 layout->addWidget(meLineEdits[i], i + 1, 1);
								...
								meDatabaseColumns[i].getType().toUpper().compare("TIMESTAMP(3)[]") == 0)
					 {
                         meIceClientLogging->insertItem("10",QString(QHostInfo::localHostName()),"2UVServerTest.exe","clEditObjectUI::createGridGroupBox() -> meListWidgetExtendString");
						 meLabels[i] = new QLabel(meDatabaseColumns[i].getName());
					 ...
					 {
                         meIceClientLogging->insertItem("10",QString(QHostInfo::localHostName()),"2UVServerTest.exe","clEditObjectUI::createGridGroupBox() -> meDateTimeEdit");
						 meLabels[i] = new QLabel(meDatabaseColumns[i].getName());
						 
						 QDateTime loDateTime = QDateTime::fromString(paValues.at(i).c_str(), "yyyy-MM-dd hh:mm:ss");
						 meDateTimeEdit[i] = new QDateTimeEdit(loDateTime);
						 meDateTimeEdit[i]->setDisplayFormat("yyyy-MM-dd hh:mm:ss.zzz");
					 ...
					 else if (meDatabaseColumns[i].getType().toUpper().compare("TIMESTAMP(3)") == 0)
					 {
                         meIceClientLogging->insertItem("10",QString(QHostInfo::localHostName()),"2UVServerTest.exe","clEditObjectUI::createGridGroupBox() -> meDateTimeEdit");
						 meLabels[i] = new QLabel(meDatabaseColumns[i].getName());
						 
						 QDateTime loDateTime = QDateTime::fromString(paValues.at(i).c_str(), "yyyy-MM-dd hh:mm:ss.zzz");
						 meDateTimeEdit[i] = new QDateTimeEdit(loDateTime);
						 meDateTimeEdit[i]->setDisplayFormat("yyyy-MM-dd hh:mm:ss.zzz");
					 ...
					 else if (meDatabaseColumns[i].getType().toUpper().compare("UUID") == 0)
					 {
                         ...
                         if (QString(paValues.at(i).c_str()).trimmed().compare("") == 0)
							meWidgetExtendObject[i]->meValues = QString("");
						 else
						 {
                            meWidgetExtendObject[i]->meValues = QString(paValues.at(i).c_str());
                            meWidgetExtendObject[i]->getCurrentValue();
                         }

                         meIceClientLogging->insertItem("10",QString(QHostInfo::localHostName()),"2UVServerTest.exe","clEditObjectUI::createGridGroupBox() -> tep");
						 layout->addWidget(meLabels[i], i + 1, 0);
					 ...
					 {
                         meIceClientLogging->insertItem("10",QString(QHostInfo::localHostName()),"2UVServerTest.exe","clEditObjectUI::createGridGroupBox() -> meListWidgetExtendObject");
						 meLabels[i] = new QLabel(meDatabaseColumns[i].getName());
						 ...
						 for (int k = 0; k < loElements.size(); k++)
						 {
							if (loElements.at(k).compare("") == 0)
								meIceClientLogging->insertItem("10",QString(QHostInfo::localHostName()),"2UVServerTest.exe","clEditObjectUI::createGridGroupBox() -> no value detected");
							else
							{
								meListWidgetExtendObject[i]->meListWidgetId->addItem(loElements.at(k));
								meListWidgetExtendObject[i]->getCurrentValue(loElements.at(k));
							}
						 }
								...
								meDatabaseColumns[i].getType().toUpper().compare("FLOAT8") == 0)
					 {
						 ...
						 {
                            meIceClientLogging->insertItem("10",QString(QHostInfo::localHostName()),"2UVServerTest.exe","clEditObjectUI::createGridGroupBox() -> meComboBox");
							meLabels[i] = new QLabel(meDatabaseColumns[i].getName());
						 ...
						 else
                         {
							...
							if (QString(paValues.at(i).c_str()).trimmed().compare(QString("")) == 0)
								meLineEdits[i]->setText(QString("0"));
							else
								meLineEdits[i]->setText(QString(paValues.at(i).c_str()));
    ...
    {
        meIceClientLogging->insertItem("10",QString(QHostInfo::localHostName()),"2UVServerTest.exe","clEditObjectUI::createGridGroupBox() -> " + QString(e.what()));
    }
 }
...
void clEditObjectUI::slotButtonOKPressed()
{
    try
    {
        for(int i = 0; i < NumGridRows; i++)
        {
			...
			{
				meValues[i] = new QString(meWidgetExtendObject[i]->meValues);
			}
			else if (meListWidgetExtendObject[i] != NULL)
			{
				QString loValues;
				for (int j = 0; j < meListWidgetExtendObject[i]->meListWidgetId->count(); j++)
				{
					QListWidgetItem *loListWidgetItem = meListWidgetExtendObject[i]->meListWidgetId->item(j);
