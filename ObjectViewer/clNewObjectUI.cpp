...
void clNewObjectUI::initializeElements()
{
    ...
    catch(exception &e)
    {
        ...
        meIceClientLogging->insertItem("10",QString(QHostInfo::localHostName()),"2UVServerTest.exe","clNewObjectUI::initializeElements" + QString(e.what()));
    }
}
 ...
 void clNewObjectUI::createGridGroupBox()
 {
    ...
    */
     ...
     for (int i = 0; i < (int) meDatabaseColumns.size(); ++i) {
		 cout << meDatabaseColumns[i].getType().toStdString() << endl;
         ...
         else
         {
						...
						meDatabaseColumns[i].getType().toUpper().compare("FLOAT8") == 0)
			 {
				 ...
				 if (meDatabaseColumns[i].getExtra().toUpper().compare("ENUM") == 0)
				 {
					...
					meComboBox[i]->setValue(QString("0"));
					layout->addWidget(meLabels[i], i + 1, 0);
...
void clNewObjectUI::slotButtonOKPressed()
{
    try
    {
        for(int i = 0; i < NumGridRows; i++)
        {
			...
			{
				QString loValues;
				for (int j = 0; j < (int) meListWidgetExtendObject[i]->meListWidget->count(); j++)
				{
			...
			{
				QString loValues;
				for (int j = 0; j < (int) meListWidgetExtendString[i]->meListWidget->count(); j++)
				{
    ...
    catch(exception &e)
    {
        ...
        meIceClientLogging->insertItem("10",QString(QHostInfo::localHostName()),"2UVServerTest.exe","clNewObjectUI::slotButtonOKPressed() -> " + QString(e.what()));
    }
}
...
void clNewObjectUI::slotButtonCancelPressed()
{
    ...
    catch(exception &e)
    {
        ...
        meIceClientLogging->insertItem("10",QString(QHostInfo::localHostName()),"2UVServerTest.exe","clNewObjectUI::slotButtonCancelPressed() -> " + QString(e.what()));
    }
}
